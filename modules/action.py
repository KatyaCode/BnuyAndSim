    ...
    def perform(self, targets):
        ...
        if self.base_damage > 0:
            for target in targets:
                ...
                hit = 0
                while hit < num_hits:
                    damage_dealt = target.receive_action_damage(self.damage() * charge_multiplier, self.character, self.name)
                    hit_applied_debuff = self.trigger_on_hit_effects(target, damage_dealt)
        ...
        for effect in self.effects:
            if effect.effect_type == 'ON_HIT':
                if isinstance(effect, effects.AppliesDebuffOnHit):
                    success = effect.trigger(self.character, target, damage_dealt)
