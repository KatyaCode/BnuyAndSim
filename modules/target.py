...
class Target:
    ...
    def receive_action_damage(self, action, character, charge_multiplier=Decimal(1)):
        ...
        actual_damage = self.calculate_variance(
            action.damage * charge_multiplier * damage_multiplier, character)

        self.apply_damage(actual_damage, character.name, action.name)
    ...
    def calculate_variance(self, damage, character):
        ...
        crit_multiplier = character.crit_multiplier if random.random(
        ) <= character.crit_chance else Decimal(1)
